...
fn test_hot_cold_loading_performance() {
    ...
    for i in 0..num_files {
        ...
        let content = if i % 3 == 0 {
            ...
            format!("File {} discusses user interfaces, web applications and design patterns", i)
        } else {
    ...
    let search_terms = [
        "document", "technical", "algorithm", "database", "performance",
        "interface", "application", "design", "pattern",
        "architecture", "network", "protocol", "security"
    ];
    ...
    println!("Cold search median: {:?}", median_duration(&cold_search_durations));
    println!("Hot search median: {:?}", median_duration(&hot_search_durations));
